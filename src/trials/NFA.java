...
public class NFA
{
     ...
     */
    public static final NFA or(NFA nfa1, NFA nfa2) {
        nfa1.exit.isFinal = false ;
        nfa2.exit.isFinal = false ;
        NFAState entry = new NFAState() ;
        NFAState exit  = new NFAState() ;
        exit.isFinal = true ;
        entry.addEmptyEdge(nfa1.entry) ;
        entry.addEmptyEdge(nfa2.entry) ;
        nfa1.exit.addEmptyEdge(exit) ;
        nfa2.exit.addEmptyEdge(exit) ;
        return new NFA(entry,exit) ;
    }
    ...
    public static void main(String[] args) {
        ...
        pat.entry.displayTransitions();
        System.out.println("EXIT:");
        pat.exit.displayTransitions();
